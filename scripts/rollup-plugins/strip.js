...
function strip(options = {}) {
  ...
  console.info("createFilter:", createFilter, include, exclude);
  ...
  const functions = (
    options.functions || ["console.*", "assert.*"]
  ).map((keypath) =>
  ...
  const firstPassFilter =
    firstPass.length > 0 ? (code) => reFirstpass.test(code) : () => false;
